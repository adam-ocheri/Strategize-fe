...
const API_URL = process.env.NODE_ENV === 'production' ? 'https://strategize-be.vercel.app/api/project/ltgs/' : 'http://localhost:4000/api/project/ltgs/';
//! ROUTE: api/LTGs -----------------------------------------------------------------------------------------------------------------------
...
export const getAll = async ({ parentId, token }) => {
...
export const getOne = async ({ id, parentId, token }) => {
    (console.log('trying to get all LTGs...'));
    console.log(token);
    const authConfig = {
        headers: {
...
export const deleteById = async ({ id, parentId, owner, token }) => {
    (console.log('trying to get all LTGs...'));
    console.log(token);
    const authConfig = {
        headers: {
...
export const getAllSubstations = async ({ id, parentId, owner, token }) => {
    (console.log('trying to get all LTGs...'));
    console.log(token);
    const authConfig = {
        headers: {
