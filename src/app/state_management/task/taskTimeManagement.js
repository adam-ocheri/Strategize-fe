...
export default function TaskTimeManagement({ user, item, type }) {
    const dispatch = useAppDispatch();
    const [timeType, setTimeType] = useState(type);
    ...
    // Update Task Time
    const updateTaskTime = async (date, newTime, id, parentId) => {
    ...
    return (_jsx("div", { style: { maxWidth: '200px' }, children: _jsx("input", { className: 'mb5 time-input jt-center font-11', type: 'time', value: time, onChange: async (t) => updateTaskTime(item.date, t.target.value, item._id, item.owningObjective) }) }));
}
