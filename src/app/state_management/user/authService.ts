...
const API_URL_Stat = 'http://localhost:4000/api/user/stats';
...
export const updateStat = async ({stat, targetValue, id, token} : any) => {
    ...
    const response = await axios.post(API_URL_Stat + `?id=${id}`, {stat, targetValue}, authConfig);
